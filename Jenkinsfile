    ...
    stage('Package') {
      steps {
        ...
        container('maven') {
          ...
          Package
          ----------------------------------------"""
          sh "mvn ${MAVEN_ARGS} -T4C -f server/pom.xml -DskipTests install"
          sh "mvn ${MAVEN_ARGS} -T4C -f packages/pom.xml -DskipTests install"
        }
      }
      post {
        success {
    ...
    stage('Deploy Maven artifacts') {
      steps {
        ...
        container('maven') {
          ...
          ----------------------------------------"""
          sh "mvn ${MAVEN_ARGS} -T1C -Pdistrib -DskipTests deploy"
        }
      }
      post {
        success {
